...
class GmeQwen2VL:
    ...
    def forward(
        ...
        **kwargs
        ...
        # --- transformers >=4.5x compatibility patch (jolnon) ---
        # The original GME forward manually spliced visual embeds into
        # `self.base.model.embed_tokens(...)` then ran `self.base.model(...)`.
        # In current transformers, `Qwen2VLModel` (=self.base.model) itself
        # accepts input_ids + pixel_values + image_grid_thw and handles vision-
        # token replacement *and* mrope position ids internally, returning
        # last_hidden_state. We delegate to it so behaviour matches the model's
        # native pipeline, then apply GME's last-token pooling unchanged.
        if attention_mask is not None:
            attention_mask = attention_mask.to(self.base.device)
